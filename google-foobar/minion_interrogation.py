...
def answer(minions):
    # your code here
    # Greedy seems fine, we can prove both Greedy Choice Property and Optimal Substructure Property
    temp = []
    for i in range(len(minions)):
        temp.append({"idx": i, "item": minions[i]})
        
    temp.sort(key = lambda x : float(x["item"][0]) / x["item"][1] * x["item"][2])
    return [i["idx"] for i in temp]
