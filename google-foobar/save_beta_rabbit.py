...
def answer(food, grid):
    ...
    matrix = [[[-1 for x in range(maxfood)] for y in range(size)] for z in range(size)]
    
    cnt = grid[size - 1][size - 1]
    for i in range(grid[size - 1][size - 1], maxfood):
        matrix[size - 1][size - 1][i] = i - grid[size - 1][size - 1]
    
    for i in range(2, size + 1):
        cnt += grid[size - 1][size - i]
        for j in range(cnt, maxfood):
            matrix[size - 1][size - i][j] = j - cnt
        
    cnt = grid[size - 1][size - 1]
    for i in range(2, size + 1):
        cnt += grid[size - i][size - 1]
        for j in range(cnt, maxfood):
            matrix[size - i][size - 1][j] = j - cnt
        
    for i in range(2, size + 1):
        for j in range(2, size + 1):
            for k in range(grid[size - i][size - j], maxfood):
                remains = k - grid[size - i][size - j]
                one = matrix[size - i][size - j + 1][remains]
                two = matrix[size - i + 1][size - j][remains]
                if one >= 0 and two >= 0:
                    matrix[size - i][size - j][k] = min(one, two)
                else:
                    matrix[size - i][size - j][k] = max(one, two)

    return matrix[0][0][food]
